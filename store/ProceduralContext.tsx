...
import { TemplateMetadata, MappingContext, TransformedPayload, KnowledgeContext, KnowledgeRegistry, FeedbackStrategy, FeedbackRegistry, SerializableLayer } from '../types';
import { calculateGroupBounds } from '../services/psdService';
...
interface ProceduralContextType extends ProceduralState {
  ...
  clearReviewerPayload: (nodeId: string, handleId: string) => void;
...
export const ProceduralStoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const registerResolved = useCallback((nodeId: string, handleId: string, context: MappingContext) => {
    setResolvedRegistry(prev => {
      ...
      // PHASE 1: METRIC EXPANSION (The AABB Pre-pass)
      // Calculate visual meat bounds immediately upon resolution to ensure subsequent logic
      // (like the Remapper) operates on the collective bounds rather than individual layer coordinates.
      const contentBounds = calculateGroupBounds(context.layers as SerializableLayer[]);
      const enrichedContext = { ...context, contentBounds };
      
      if (currentContext && JSON.stringify(currentContext) === JSON.stringify(enrichedContext)) return prev;
      
      return {
        ...prev,
        [nodeId]: {
          ...nodeRecord,
          [handleId]: enrichedContext
        }
      };
    });
  }, []);
  ...
  const clearReviewerPayload = useCallback((nodeId: string, handleId: string) => {
    setReviewerRegistry(prev => {
  ...
  }, [resolvedRegistry, payloadRegistry, reviewerRegistry, previewRegistry, feedbackRegistry]);
  ...
  const resetProject = useCallback(() => {
    console.log("[ProceduralContext] Resetting Project State (Hard Clean)...");
    ...
    setPreviewRegistry({});
    setFeedbackRegistry({});
    setKnowledgeRegistry({});
    
    // 2. Clear Singleton Service
    SmartObjectRegistry.clear();
    console.log("[ProceduralContext] SmartObjectRegistry Cleared.");

    // 3. Bump Version to force downstream refetch
    setGlobalVersion(v => v + 1);
    
    console.log("[ProceduralContext] Reset Complete.");
  }, []);
  ...
  const value = useMemo(() => ({
    ...
    psdRegistry, templateRegistry, resolvedRegistry, payloadRegistry, reviewerRegistry, previewRegistry, feedbackRegistry, knowledgeRegistry, globalVersion,
    registerPsd, registerTemplate, registerResolved, registerPayload, registerReviewerPayload, clearReviewerPayload, registerPreviewPayload, updatePayload, registerFeedback, clearFeedback, registerKnowledge, updatePreview,
    unregisterNode, flushPipelineInstance, triggerGlobalRefresh, resetProject
  ]);
