...
import { TemplateMetadata, ContainerDefinition, DesignValidationReport, ValidationIssue, SerializableLayer, ContainerContext, TransformedPayload, TransformedLayer, OpticalMetrics, BoundingBox } from '../types';
...
export const getCleanLayerTree = (layers: Layer[], path: string = ''): SerializableLayer[] => {
  ...
  layers.forEach((child, index) => {
    ...
    // PHASE 1: SMART OBJECT METADATA EXTRACTION
    if (child.placedLayer) {
      node.smartObject = {
        id: child.placedLayer.id,
        transform: child.placedLayer.transform,
        type: child.placedLayer.type
      };
    }
...
/**
 * Calculates the Axis-Aligned Bounding Box (AABB) or "Visual Meat" of a layer group.
 * Phase 1: Metric Expansion (AABB Pre-pass).
 * Recursively traverses the layer tree to find the absolute minimum and maximum coordinates.
 * 
 * @param layers The array of SerializableLayers to traverse.
 * @returns A BoundingBox object representing the collective bounds.
 */
export const calculateGroupBounds = (layers: SerializableLayer[]): BoundingBox => {
    let minX = Infinity;
    let minY = Infinity;
    let maxX = -Infinity;
    let maxY = -Infinity;
    ...
    const traverse = (nodes: SerializableLayer[]) => {
        for (const node of nodes) {
            // We include all visible layers in the "Visual Meat" calculation.
            if (node.isVisible) {
                // Leaf nodes (layers or generative) contribute to bounds if they have dimensions.
                // Critical: Use actual dimensions. If a layer has w=0 or h=0, it contributes nothing visually.
                if (node.type !== 'group') {
                    const { x, y, w, h } = node.coords;
                    if (w > 0 && h > 0) {
                        if (x < minX) minX = x;
                        if (y < minY) minY = y;
    ...
    if (!hasContent) {
        // Return reasonable default box if empty to prevent division-by-zero/infinity in Remapper
        // Standard placeholder size: 100x100
 ...
 * 
 * UPDATE: Implements Recursive Bound Resolution for Group Layers.
 * Group folders often have misleading metadata (e.g. 0,0 coords).
 * We calculate the union of visible children to determine true bounds.
 */
    ...
    if (group.name && containerMap.has(group.name)) {
        ...
        group.children?.forEach(layer => {
            ...
            if (typeof layer.top === 'number' && typeof layer.left === 'number' && 
                ...
                let layerLeft = layer.left;
                let layerTop = layer.top;
                let layerRight = layer.right;
                let layerBottom = layer.bottom;

                // RECURSIVE RESOLUTION for Groups
                // If this layer is a group, we ignore its folder metadata and compute the visual union.
                if (layer.children && Array.isArray(layer.children) && layer.children.length > 0) {
                    // Convert to lightweight tree to use our existing bound calculator
                    // This ensures consistent logic with the Analyst node
                    const cleanTree = getCleanLayerTree(layer.children);
                    const tightBounds = calculateGroupBounds(cleanTree);
                    
                    // Only override if we found valid content (avoid flagging empty groups falsely)
                    if (tightBounds.w > 0 && tightBounds.h > 0) {
                        layerLeft = tightBounds.x;
                        layerTop = tightBounds.y;
                        layerRight = tightBounds.x + tightBounds.w;
                        layerBottom = tightBounds.y + tightBounds.h;
                    }
                }
                ...
                const isViolation = 
                    layerLeft < container.bounds.x ||
                    layerTop < container.bounds.y ||
                    layerRight > containerRight ||
                    layerBottom > containerBottom;
 ...
 * @param linkedFiles Optional array of linked files (smart objects) to include.
 */
export const writePsdFile = async (psd: Psd, filename: string, linkedFiles?: any[]) => {
  try {
    if (linkedFiles) {
    ...
    const buffer = writePsd(psd, writeOptions);
...
/**
 * Reconciles the Affine Transformation Matrix of a Smart Object based on the
 * procedural resize and movement logic applied by the Engine.
 * 
 * Standard Matrix [a, b, c, d, tx, ty]:
 * - a (m[0]): Scale X / Skew Y
 * - b (m[1]): Skew X / Scale X (Rotated)
 * - c (m[2]): Skew Y / Scale Y (Rotated)
 * - d (m[3]): Scale Y / Skew X
 * - tx (m[4]): Translation X
 * - ty (m[5]): Translation Y
 * 
 * @param matrix The original 6-element matrix array from ag-psd.
 * @param currentBounds The original geometric bounds of the layer.
 * @param targetBounds The calculated target bounds after procedural processing.
 * @returns A new 6-element array representing the updated matrix.
 * @throws Error if the calculation results in NaN or Infinity.
 */
export const reconcileSmartObjectMatrix = (
  matrix: number[], 
  currentBounds: BoundingBox, 
  targetBounds: BoundingBox
): number[] => {
  // 1. Calculate Scale Factors (Prevent Infinity)
  // Zero-Size Guard: If current dimensions are invalid, we default scale to 1 to avoid division by zero.
  const scaleX = currentBounds.w > 0 ? targetBounds.w / currentBounds.w : 1;
  const scaleY = currentBounds.h > 0 ? targetBounds.h / currentBounds.h : 1;

  // 2. Calculate Position Delta
  const dx = targetBounds.x - currentBounds.x;
  const dy = targetBounds.y - currentBounds.y;

  // 3. Clone and Update Matrix
  // We multiply the vector components to apply scaling logic inherent to the object's transform
  const newMatrix = [...matrix];

  // Apply Scale to X-Vector (a, b)
  newMatrix[0] *= scaleX;
  newMatrix[1] *= scaleX;

  // Apply Scale to Y-Vector (c, d)
  newMatrix[2] *= scaleY;
  newMatrix[3] *= scaleY;

  // Apply Translation Delta (tx, ty)
  newMatrix[4] += dx;
  newMatrix[5] += dy;

  // 4. NaN Guard (Output Safety)
  if (newMatrix.some(v => !Number.isFinite(v))) {
      throw new Error("Matrix calculation resulted in NaN/Infinity");
  }
...
/**
 * Constructs a lightweight ag-psd object structure ready for export,
 * applying procedural transformations and reconciling Smart Object metadata.
 * 
 * @param payload The transformed layout instructions.
 * @param originalPsd The source PSD containing binary assets and original coordinates.
 * @returns A new Psd object structure.
 */
export const constructExportablePsd = (payload: TransformedPayload, originalPsd: Psd): Psd => {
  // Determine target dimensions
  const width = payload.metrics.target.w;
  const height = payload.metrics.target.h;

  const processLayer = (layer: TransformedLayer): Layer | null => {
    const { x, y, w, h } = layer.coords;
    
    // GUARD 1: Missing Source Logic
    // We need the original layer to get pixel data or children structure
    const originalLayer = findLayerByPath(originalPsd, layer.id);

    if (!originalLayer) {
        console.warn(`[Export] Skipping missing source layer ID: ${layer.id}`);
        return null;
    }
    ...
    const newLayer: Layer = {
      ...
      opacity: layer.opacity * 255, // Convert 0-1 to 0-255
    ...
    // 2. Smart Object Logic
    let isSmartObjectProcessed = false;

    if (layer.smartObject) {
        try {
            // Calculate Original Bounds from source layer coordinates
            const origW = (originalLayer.right || 0) - (originalLayer.left || 0);
            const origH = (originalLayer.bottom || 0) - (originalLayer.top || 0);
            ...
            const targetBounds: BoundingBox = { x, y, w, h };

            // Reconcile Matrix
            const newTransform = reconcileSmartObjectMatrix(
                layer.smartObject.transform,
                originalBounds,
                targetBounds
            );

            newLayer.placedLayer = {
                id: layer.smartObject.id,
                type: layer.smartObject.type as any,
            ...
            isSmartObjectProcessed = true;

        } catch (e) {
            console.warn("Smart Object reconciliation failed for layer:", layer.name, e);
            // Fall through to raster logic via flag
        }
    } 
    
    // 3. Raster Logic (Standard Layer)
    // GUARD 3: Fallback Logic
    // If layer.type !== 'group', copy sourceLayer.canvas or sourceLayer.imageData
    if (!isSmartObjectProcessed && layer.type !== 'group') {
        if (originalLayer.canvas) newLayer.canvas = originalLayer.canvas;
        if (originalLayer.imageData) newLayer.imageData = originalLayer.imageData;
    }
  ...
  return {
    width,
    height,
    children: payload.layers.map(processLayer).filter((l): l is Layer => l !== null)
  };
};

/**
 * Prunes the linkedFiles array to only include smart objects that are actually referenced
 * in the final layer hierarchy. This prevents "Program Error" in Photoshop caused by orphaned data.
 * 
 * @param layers The final layer tree structure being exported.
 * @param linkedFiles The complete registry of smart objects from the source.
 * @returns A filtered array of linked files containing only active references.
 */
export const pruneLinkedFiles = (layers: Layer[], linkedFiles: any[]): any[] => {
    ...
    const traverse = (nodes: Layer[]) => {
        for (const node of nodes) {
            if (node.placedLayer && node.placedLayer.id) {
                activeIds.add(node.placedLayer.id);
            }
            if (node.children) {
                traverse(node.children);
            }
    ...
    const filtered = linkedFiles.filter(item => activeIds.has(item.id));
    console.log(`[Prune] Optimized linkedFiles: ${linkedFiles.length} -> ${filtered.length}`);
    
    return filtered;
};

/**
 * Calculates global scaling factors between source and target dimensions.
 * Used for "Skeleton Injection" to ensure forgotten assets don't drift off-canvas.
 * 
 * @param sourceDims Original dimensions { w, h }
 * @param targetDims New dimensions { w, h }
 * @returns Scale factors { x, y }
 */
export const getGlobalScaleFactors = (
    sourceDims: { w: number, h: number }, 
    targetDims: { w: number, h: number }
): { x: number, y: number } => {
    if (sourceDims.w <= 0 || sourceDims.h <= 0) {
        return { x: 1, y: 1 };
    }

    return {
        x: targetDims.w / sourceDims.w,
        y: targetDims.h / sourceDims.h
    };
};
