...
import { createContainerContext, calculateGroupBounds } from '../services/psdService';
...
export const ContainerResolverNode = memo(({ id, data }: NodeProps<PSDNodeData>) => {
  ...
  const channels: ChannelState[] = useMemo(() => {
    return Array.from({ length: channelCount }).map((_, index) => {
      ...
      // PHASE 2.1: Robust Bounds Calculation
      let contentBounds = { x: 0, y: 0, w: 100, h: 100 }; // Safe Default
      if (result.layer && result.layer.children) {
          contentBounds = calculateGroupBounds(result.layer.children);
          console.log(`[Resolver] Bounds for ${containerContext.containerName}:`, contentBounds);
      }
      ...
      return {
        ...
        resolvedContext: result.layer && containerContext ? {
            ...
            status: 'resolved',
            message: result.message,
            contentBounds: contentBounds // Attach calculated visual bounds
        } : null
      };
    });
