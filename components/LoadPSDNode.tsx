...
export const LoadPSDNode = memo(({ data, id }: NodeProps<PSDNodeData>) => {
  ...
  const handleFileChange = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // CHECKROOM PATTERN: Extract linkedFiles (Smart Object Binaries) and move to Registry
      let smartObjectReferenceId: string | undefined;
      const linkedFiles = parsedPsd.linkedFiles;

      if (linkedFiles && linkedFiles.length > 0) {
          const refId = crypto.randomUUID(); // Modern browser UUID generation
          SmartObjectRegistry.set(refId, linkedFiles);
          smartObjectReferenceId = refId;
          console.log(`[LoadPSD] Offloaded ${linkedFiles.length} Smart Objects to Registry (ID: ${refId})`);
          
          // Clear heavy data from the object to avoid duplication in psdRegistry if referenced
          // Note: psdRegistry is in Context state, which is okay, but we definitely don't want it in Flow state.
          // We keep it on parsedPsd for now as the Context might need it, but the Node Data definitely won't get it.
      }
          ...
          if (node.id === id) {
            return {
              ...
              data: {
                ...
                smartObjectReferenceId: smartObjectReferenceId, // Lightweight Ticket
                // linkedFiles is intentionally OMITTED here to prevent DataCloneError
                error: null,
              },
            };
          }
