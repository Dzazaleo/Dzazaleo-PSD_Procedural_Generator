...
const calculateOverrideMetrics = (
    ...
    // Engine Detection for Metrics
    // We must replicate the Engine logic here to show accurate 'geomX/Y' (Before Override) vs 'finalX/Y' (After Override)
    // FIX: Using spatialLayout instead of layoutStrategy as per types.ts definition
    const engineMode = strategy.spatialLayout || 'UNIFIED_FIT';

    // Base Calculations
    const sourceW = Math.max(sourceRect.w, 1);
    const sourceH = Math.max(sourceRect.h, 1);
    ...
    if (engineMode === 'STRETCH_FILL') {
        baseXScale = targetRect.w / sourceW;
        baseYScale = targetRect.h / sourceH;
        baseOffsetX = 0;
        baseOffsetY = 0;
    } else {
        // UNIFIED_FIT or ABSOLUTE_PIN (Default base)
        const unifiedScale = Math.min(targetRect.w / sourceW, targetRect.h / sourceH);
        // Clamp scale to prevent explosion
        const effectiveScale = Math.min(unifiedScale, 5.0);
        
        baseXScale = effectiveScale;
        baseYScale = effectiveScale;
        
        const centeredW = sourceW * effectiveScale;
        const centeredH = sourceH * effectiveScale;
        baseOffsetX = (targetRect.w - centeredW) / 2;
        baseOffsetY = (targetRect.h - centeredH) / 2;
    }
    ...
    const traverse = (layers: SerializableLayer[]) => {
        layers.forEach(layer => {
            ...
            if (override) {
                // 1. Calculate Local Coordinates (Source Space)
                ...
                // 3. Apply Override (Final Position)
                // Note: The override offsets from the Analyst are typically relative to the Target Top-Left
                // BUT, our physics engine applies them as absolute placements if ABSOLUTE_PIN, 
                // or deltas if UNIFIED_FIT. 
                // To keep metrics simple, we show the delta between 'Geometric Default' and 'Final'.
...
const RemapperInstanceRow = memo(({ 
    ...
    // Phase 3: Triangulation & Mode Visualization
                  ...
                  <div className="flex flex-col w-full pr-4">
                      <div className="flex justify-between items-center">
                          <div className="flex items-center space-x-2">
                              {/* MODE BADGE */}
                              {instance.isSemanticMode ? (
                                  <div className="flex items-center gap-1 bg-indigo-500/20 text-indigo-300 px-1.5 py-0.5 rounded border border-indigo-500/40" title="Semantic Mode Active">
                                      <Brain className="w-3 h-3" />
                                      <span className="text-[8px] font-bold tracking-wider">SEMANTIC</span>
                                  </div>
                              ) : (
                                  <div className="flex items-center gap-1 bg-slate-700 text-slate-400 px-1.5 py-0.5 rounded border border-slate-600" title="Geometric Mode (Passive)">
                                      <Box className="w-3 h-3" />
                                      <span className="text-[8px] font-bold tracking-wider">GEOMETRIC</span>
                                  </div>
                              )}
        ...
        for (let i = 0; i < instanceCount; i++) {
            ...
            if (sourceData.ready && targetData.ready) {
                ...
                if (feedback && feedback.overrides.length > 0) {
                     ...
                     // FIX: Explicitly type generic parameters for Map to avoid 'unknown' type inference on values
                     ...
                     effectiveStrategy = { ...(sourceData.aiStrategy || {}), overrides: mergedOverrides } as LayoutStrategy;
                }
                ...
                // --- HYBRID LAYOUT PROTOCOL ENGINES ---
                // Default to UNIFIED_FIT if no strategy or explicitly requested
                // FIX: Used strategy.spatialLayout as defined in LayoutStrategy type
                const layoutMode = isSemanticMode && strategy?.spatialLayout ? strategy.spatialLayout : 'UNIFIED_FIT';
                
                // Base Transforms for the Group
                const sourceW = Math.max(sourceRect.w, 1);
                const sourceH = Math.max(sourceRect.h, 1);
                ...
                // ENGINE A: UNIFIED_FIT (Safe/Geometric)
                if (layoutMode === 'UNIFIED_FIT' || layoutMode === 'ABSOLUTE_PIN') {
                    // Calculate strict Aspect-Fit
                    const unifiedScale = Math.min(targetRect.w / sourceW, targetRect.h / sourceH);
                    // Clamp for safety
                    const effectiveScale = Math.min(unifiedScale, 5.0);
                ...
                // ENGINE B: STRETCH_FILL (Backgrounds)
                else if (layoutMode === 'STRETCH_FILL') {
                    baseXScale = targetRect.w / sourceW;
                    baseYScale = targetRect.h / sourceH;
                    baseOffsetX = 0;
                    baseOffsetY = 0;
                ...
                // Global Scale for metadata (approximate)
                let globalScale = baseXScale;
                ...
                const transformLayers = (layers: SerializableLayer[], depth = 0, parentDeltaX = 0, parentDeltaY = 0): TransformedLayer[] => {
                    ...
                    // 4. Physics Engine (Only if Semantic Mode & Root Level)
                    if (depth === 0 && strategy && isSemanticMode) {
                        
                        // A. GRID SOLVER (Only 'flow' items)
                        const gridCandidates = transformed.filter(l => getOverride(l.id)?.layoutRole === 'flow');
